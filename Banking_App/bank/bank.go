...
func (bm *BankManager) UpdateBankFieldByID(caller *customer.Customer, bankID int, field string, value interface{}) error {
	if !isAuthorized(caller) {
		return apperror.NewAuthError("update bank: caller not authorized") // Corrected: returns single error
	}
...
func (bm *BankManager) SoftDeleteBank(
	...
	if !isAuthorized(caller) {
		return apperror.NewAuthError("soft delete bank: caller not authorized") // Corrected: returns single error
	}
