...
// hasActiveAccounts: func to check if the bank has active accounts
...
func (bank *Bank) softDelete(hasActiveAccounts func(bankID int) (bool, error)) error {
	if !bank.IsActive {
		return apperror.NewBankError("delete", "bank already inactive")
	}
	hasActive, err := hasActiveAccounts(bank.BankID)
	if err != nil {
		return err
	}
	if hasActive {
