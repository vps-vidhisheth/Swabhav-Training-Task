...
func main() {
	// Create a bank
	bank1, err := bank.CreateBank("First Bank of Go")
	if err != nil {
	...
	custAccounts, err := account.GetCustomerAccounts(cust1.CustomerID, true)
	if err != nil {
		fmt.Println("Error getting customer accounts:", err)
		return
	}
	fmt.Printf("Accounts for customer %s:\n", cust1.FirstName)
	for _, acc := range custAccounts {
