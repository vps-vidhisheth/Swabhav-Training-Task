...
func main() {
	...
	account.SetCustomerLookup(func(id int) (*customer.Customer, error) {
		return customerManager.GetByID(admin, id)
	})
	...
	// Create accounts
	account1, _ := account.CreateAccount(admin, customer1, bank1)
	account2, _ := account.CreateAccount(admin, customer1, bank2)
	account3, _ := account.CreateAccount(admin, customer2, bank1)
	...
	fmt.Printf("Transferred Rs %.2f between accounts %d and %d\n", 100.0, account1.AccountID, account2.AccountID)

	account.AdminTransfer(admin, account1, account3, 50)
	fmt.Printf("Admin transferred Rs %.2f from account %d to account %d\n", 50.0, account1.AccountID, account3.AccountID)
	...
	// Interbank transfers
	fmt.Println("\nInterbank Transfers")
	account.AdminTransfer(admin, account1, account3, 100)
	fmt.Printf("Transferred Rs %.2f within bank %d\n", 100.0, bank1.BankID)

	account.AdminTransfer(admin, account1, account2, 150)
	fmt.Printf("Transferred Rs %.2f from bank %d to bank %d\n", 150.0, bank1.BankID, bank2.BankID)
	...
	for _, b := range []*bank.Bank{bank1, bank2} {
		...
		if err != nil {
			fmt.Printf("Error for bank %d (%s): %v\n", b.BankID, b.Name, err)
			continue
		}
